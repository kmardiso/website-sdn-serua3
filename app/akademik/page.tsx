...
const BULAN = ['Januari', 'Februari', 'Maret', 'April', 'Mei', 'Juni', 'Juli', 'Agustus', 'September', 'Oktober', 'November', 'Desember']
const HARI = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab']
const HARI_MERAH: Record<string, number[]> = { '0': [1], '2': [29], '4': [1], '7': [17], '11': [25] }
...
export default function AkademikPage() {
  ...
  const firstDay = new Date(currentYear, currentMonth, 1).getDay()
  const daysInMonth = new Date(currentYear, currentMonth + 1, 0).getDate()
  const eventsBulanIni = kalenderData[selectedSemester].filter(e => selectedSemester === 'Hari Normal' || e.bulan === currentMonth + 1)
  const selectedEvents = selectedDay ? kalenderData[selectedSemester].filter(e => {
    const dayNum = parseInt(e.tanggal.split(' ')[0])
    return dayNum === selectedDay && (selectedSemester === 'Hari Normal' || e.bulan === currentMonth + 1)
  }) : []
  const eventDays = new Set(kalenderData[selectedSemester].filter(e => selectedSemester === 'Hari Normal' || e.bulan === currentMonth + 1).map(e => parseInt(e.tanggal.split(' ')[0])).filter(n => !isNaN(n)))
  const hariMerahBulan = HARI_MERAH[String(currentMonth)] || []
  ...
  return (
    <>
      <style>{`
        .akademik-hero-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 3rem; align-items: center; }
        .akademik-hero-img { display: block; }
        .pilar-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 1.5rem; }
        .kalender-grid { display: grid; grid-template-columns: 1fr 300px; gap: 2rem; }
        .metode-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 3rem; align-items: center; }
        .metode-img { display: block; }
        @media (max-width: 768px) {
          .akademik-hero-grid { grid-template-columns: 1fr; }
          .akademik-hero-img { display: none; }
          .pilar-grid { grid-template-columns: 1fr; }
          .kalender-grid { grid-template-columns: 1fr; }
          .metode-grid { grid-template-columns: 1fr; }
          .metode-img { display: none; }
          .semester-filter { flex-wrap: wrap; }
        }
      `}</style>
      ...
      <section style={{ background: 'linear-gradient(135deg, #1B2D6B, #2a4090)', padding: '3rem 1.5rem' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div className="akademik-hero-grid">
            <div>
              <div style={{ fontSize: 12, color: 'rgba(255,255,255,0.6)', marginBottom: 8 }}>Beranda &rsaquo; Akademik</div>
              <div style={{ display: 'inline-block', background: 'rgba(200,168,75,0.2)', color: '#C8A84B', fontSize: 12, fontWeight: 600, padding: '5px 14px', borderRadius: 20, marginBottom: 16 }}>Kurikulum Merdeka</div>
              <h1 style={{ fontWeight: 800, fontSize: '2rem', color: '#fff', lineHeight: 1.2, marginBottom: 16 }}>
                Membentuk Generasi <span style={{ color: '#C8A84B' }}>Berkarakter</span> dengan Kurikulum Merdeka
              </h1>
              <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 14, lineHeight: 1.8, marginBottom: 24 }}>Kami menerapkan kurikulum inovatif yang berpusat pada pengembangan karakter, kreativitas, dan kompetensi siswa.</p>
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <Link href="/kontak" style={{ background: '#C8A84B', color: '#1B2D6B', fontWeight: 700, fontSize: 14, padding: '12px 24px', borderRadius: 10, textDecoration: 'none' }}>Daftar Sekarang</Link>
                <Link href="/profil" style={{ background: 'rgba(255,255,255,0.1)', color: '#fff', fontWeight: 600, fontSize: 14, padding: '12px 24px', borderRadius: 10, border: '1px solid rgba(255,255,255,0.2)', textDecoration: 'none' }}>Pelajari Lebih</Link>
              </div>
            </div>
      ...
      <section style={{ padding: '4rem 1.5rem', background: '#fff' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          ...
          <div className="pilar-grid">
            {[
              { icon: '🎯', title: 'Pembelajaran Berdiferensiasi', desc: 'Setiap siswa mendapatkan pendekatan belajar yang disesuaikan dengan kebutuhan, bakat, dan minat.' },
              { icon: '🤝', title: 'Projek Profil Pancasila', desc: 'Pengembangan karakter melalui projek nyata yang mencerminkan nilai-nilai Pancasila.' },
              { icon: '💡', title: 'Asesmen Autentik', desc: 'Asesmen yang berfokus pada pertumbuhan dan perkembangan siswa, bukan hanya hasil akhir.' },
            ].map((p, i) => (
              <div key={i} style={{ background: '#f9fafb', borderRadius: 16, padding: '2rem', textAlign: 'center' }}>
                <div style={{ fontSize: 40, marginBottom: 16 }}>{p.icon}</div>
                <h3 style={{ fontWeight: 700, fontSize: 15, color: '#1B2D6B', marginBottom: 10 }}>{p.title}</h3>
      ...
      <section style={{ padding: '4rem 1.5rem', background: '#f9fafb' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '2rem', flexWrap: 'wrap', gap: 12 }}>
            ...
            <div className="semester-filter" style={{ display: 'flex', gap: 8 }}>
              {['Semester 1', 'Semester 2', 'Hari Normal'].map(t => (
                <button key={t} onClick={() => { setSelectedSemester(t); setSelectedDay(null) }}
                  style={{ padding: '8px 16px', borderRadius: 8, fontSize: 13, cursor: 'pointer', fontWeight: selectedSemester === t ? 700 : 400, border: selectedSemester === t ? '2px solid #1B2D6B' : '1px solid #e5e7eb', background: selectedSemester === t ? '#1B2D6B' : '#fff', color: selectedSemester === t ? '#fff' : '#374151' }}>
                  {t}
                </button>
              ))}
            </div>
          ...
          <div className="kalender-grid">
            <div style={{ background: '#fff', borderRadius: 16, padding: '1.5rem', border: '1px solid #e5e7eb' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
                <button onClick={prevMonth} style={{ background: '#f3f4f6', border: 'none', width: 36, height: 36, borderRadius: 8, cursor: 'pointer', fontSize: 18 }}>←</button>
                <div style={{ fontWeight: 700, fontSize: 16, color: '#1B2D6B' }}>{BULAN[currentMonth]} {currentYear}</div>
                <button onClick={nextMonth} style={{ background: '#f3f4f6', border: 'none', width: 36, height: 36, borderRadius: 8, cursor: 'pointer', fontSize: 18 }}>→</button>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 4, textAlign: 'center' }}>
                {HARI.map(d => <div key={d} style={{ fontSize: 12, fontWeight: 600, color: '#9ca3af', padding: '8px 0' }}>{d}</div>)}
                ...
                {Array.from({ length: daysInMonth }).map((_, i) => {
                  ...
                  return (
                    <div key={day} onClick={() => setSelectedDay(day === selectedDay ? null : day)}
                      style={{ padding: '8px 0', fontSize: 13, borderRadius: 8, background: isSelected ? '#1B2D6B' : isTodayDay ? '#eef2fc' : 'transparent', color: isSelected ? '#fff' : isMerah || isSunday ? '#ef4444' : '#374151', cursor: 'pointer', fontWeight: isSelected || isTodayDay ? 700 : 400, position: 'relative', border: isTodayDay && !isSelected ? '2px solid #1B2D6B' : '2px solid transparent' }}>
                      {day}
                      {hasEvent && !isSelected && <span style={{ position: 'absolute', bottom: 2, left: '50%', transform: 'translateX(-50%)', width: 4, height: 4, borderRadius: '50%', background: '#C8A84B', display: 'block' }} />}
                    </div>
                  )
                })}
              </div>
              {selectedDay && selectedEvents.length > 0 && (
                <div style={{ marginTop: 16, padding: '12px 14px', background: '#f9fafb', borderRadius: 10, borderLeft: '3px solid #1B2D6B' }}>
                  <div style={{ fontSize: 12, fontWeight: 600, color: '#6b7280', marginBottom: 8 }}>Kegiatan {selectedDay} {BULAN[currentMonth]}:</div>
                  {selectedEvents.map((e, i) => <div key={i} style={{ fontSize: 13.5, color: '#1B2D6B', fontWeight: 600 }}>{e.kegiatan}</div>)}
                </div>
              )}
            </div>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              ...
              {eventsBulanIni.length > 0 ? eventsBulanIni.map((a, i) => (
                <div key={i} style={{ background: '#fff', borderRadius: 10, padding: '12px 14px', border: '1px solid #e5e7eb', display: 'flex', gap: 12, alignItems: 'center' }}
                  onMouseEnter={e => (e.currentTarget.style.borderColor = '#1B2D6B')}
                  onMouseLeave={e => (e.currentTarget.style.borderColor = '#e5e7eb')}>
              ...
              )) : <div style={{ color: '#9ca3af', fontSize: 13, textAlign: 'center', padding: '2rem 0' }}>Tidak ada kegiatan di bulan ini</div>}
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: '4rem 1.5rem', background: '#fff' }}>
        <div style={{ maxWidth: 1100, margin: '0 auto' }}>
          <div className="metode-grid">
            <div>
              <div style={{ fontSize: 12, fontWeight: 600, color: '#C8A84B', letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }}>INOVASI PEDAGOGIS</div>
              <h2 style={{ fontWeight: 800, fontSize: '1.75rem', color: '#1B2D6B', marginBottom: 16, lineHeight: 1.3 }}>Mendukung Pembelajaran yang Efektif dan Menyenangkan</h2>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
                {[
                  { icon: '🎮', title: 'Gamifikasi Pembelajaran', desc: 'Mengintegrasikan elemen permainan dalam proses belajar untuk meningkatkan motivasi.' },
                  { icon: '📱', title: 'Pembelajaran Digital', desc: 'Memanfaatkan teknologi modern untuk memperkaya pengalaman belajar siswa.' },
      ...
      <section style={{ background: 'linear-gradient(135deg, #1B2D6B, #2a4090)', padding: '4rem 1.5rem', textAlign: 'center' }}>
        <div style={{ maxWidth: 600, margin: '0 auto' }}>
          <h2 style={{ fontWeight: 800, fontSize: '1.75rem', color: '#fff', marginBottom: 16 }}>Siap Menjadi Bagian dari Keluarga SD Negeri Serua 3?</h2>
          <p style={{ color: 'rgba(255,255,255,0.75)', fontSize: 14, marginBottom: 28 }}>Bergabunglah bersama ribuan siswa berprestasi.</p>
